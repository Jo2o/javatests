...
//        System.out.println(distance);
        ...
        for (String input : firstWireInput) {
            ...
            if (input.startsWith("R")) {
                ...
                for (int i = 0; i < number; i++) {
                    stepCounter += 1;
                    Point currentWireEnd = result.get(result.size()-1);
                    result.add(new Point(currentWireEnd.x, currentWireEnd.y + 1, stepCounter));
                }
    ...
    private static List<Point> findIntersections(List<Point> firstWirePoints, List<Point> secondWirePoints) {
        List<Point> commonPoints = new ArrayList<>();
        for (Point firstWirePoint : firstWirePoints) {
            for (Point secondWirePoint : secondWirePoints) {
                if (firstWirePoint.x == secondWirePoint.x && firstWirePoint.y == secondWirePoint.y) {
                    commonPoints.add(firstWirePoint);
                    System.out.println("STEPS: " + (firstWirePoint.steps + secondWirePoint.steps));
                }
            }
        }
        return commonPoints;
